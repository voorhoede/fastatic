...
function createTable(tableData) {
	...
	const typeRows = Object.keys(tableData).map(type => createTableRow(tableData[type], type));
	const totalRow = createTotalRow(tableData);
...
function createTableRow(sizes, type) {
	const row = [
		type,
		filesize(sizes.src),
		filesize(sizes.dest),
...
function createTotalRow(tableData) {
